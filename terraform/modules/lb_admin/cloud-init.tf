# lb_admin userdata
data "cloudinit_config" "lb_admin_config" {
  gzip          = false
  base64_encode = false
  ...
  # lb_admin.cfg sourced in lb_admin script, and contains all needed variables
  part {
    content_type = "text/plain"
    content = templatefile("${path.module}/../../heat/config-scripts/lb_admin.cfg.tpl", {
      dockerhub_login          = var.dockerhub_login
      dockerhub_token          = var.dockerhub_token
      github_token             = var.github_token
      docker_registry_username = var.docker_registry_username
      docker_registry_token    = var.docker_registry_token
      k3s_master_private_ip    = var.k3s_master_private_ip
  ...
  part {
    content_type = "text/plain"
    content      = file("${path.module}/../../heat/config-scripts/worker_lb_admin_postconf.sh")
  }
}
